...
def get_snapshot_db_path(
    ...
    database = str(
        snapshot_cfg.get("database", SNAPSHOT_DB_NAME) or SNAPSHOT_DB_NAME
    ).strip()
    if not database:
...
def ensure_restore_tasks_columns(conn: sqlite3.Connection):
    ...
    if "cache_key" not in columns:
        conn.execute(
            "ALTER TABLE restore_tasks ADD COLUMN cache_key TEXT NOT NULL DEFAULT ''"
        )
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_restore_tasks_cache ON restore_tasks (cache_key, id)"
    )
...
def commit_refreshed_snapshot_cache(
    ...
    with conn:
        ...
        conn.execute(
            "UPDATE snapshots SET cache_key = ? WHERE cache_key = ?",
            (cache_key, temp_key),
        )
        conn.execute(
            """
...
def record_snapshot_refresh_failure(
    ...
    with conn:
        ...
        conn.execute(
            ...
            VALUES (?, ?, ?, ?, ?, 1, ?, ?)
            """,
            build_snapshot_refresh_failure_row(
                cache_key, existing, error, duration_seconds
            ),
        )
    return error
...
def insert_snapshot_row(
    conn: sqlite3.Connection, cache_key: str, snapshot: Dict[str, Any]
):
    ...
    conn.execute(
        ...
        (
            ...
            json.dumps(
                snapshot.get("summary") or {}, ensure_ascii=False, sort_keys=True
            ),
        ),
    )
...
def read_snapshot_page(
    ...
    with closing(open_snapshot_db(server, snapshot_cfg)) as conn:
        ...
        return {
            ...
            "invalidation_reason": meta["invalidation_reason"]
            if meta is not None
            else "",
...
def read_snapshot_count(conn: sqlite3.Connection, cache_key: str) -> int:
    row = conn.execute(
        "SELECT COUNT(*) FROM snapshots WHERE cache_key = ?", (cache_key,)
    ).fetchone()
