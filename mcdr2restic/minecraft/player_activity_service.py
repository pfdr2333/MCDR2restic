...
def handle_player_joined(app_runtime: PluginRuntime, server: PluginServerInterface, player: str):
    with app_runtime.config_state.lock:
        ...
        previous_current = non_negative_int(runtime_state.get('current_online_players', 0))
...
def record_player_left_unlocked(runtime_state: Dict[str, Any], player: str, current_online: int):
    ...
    runtime_state['last_online_check_result'] = '{} online after {} left'.format(current_online, player)
...
def record_online_sample(
    ...
    with app_runtime.config_state.lock:
        ensure_runtime(app_runtime.config_state.config)
        runtime_state = app_runtime.config_state.config['runtime']
        runtime_state['known_online_players'] = resolve_known_online_players(runtime_state, count, names)
        mark_player_activity_unlocked(runtime_state, count, 'rcon {}'.format(command), tail_text(str(result), 300))
        save_config_unlocked(app_runtime, server)


def resolve_known_online_players(runtime_state: Dict[str, Any], count: int, names) -> list:
    if names:
        return sorted(names)
    if count == 0:
        return []
...
def should_skip_for_no_player_activity(app_runtime: PluginRuntime, cfg: Dict[str, Any]) -> bool:
    if not player_activity_required(cfg):
        return False
    sample_online_players(app_runtime, app_runtime.service.server, cfg, 'schedule trigger')
    with app_runtime.config_state.lock:
        ensure_runtime(app_runtime.config_state.config)
        runtime_state = app_runtime.config_state.config['runtime']
...
def has_recent_player_activity(runtime_state: Dict[str, Any]) -> bool:
    ...
    joined = bool(
        runtime_state.get('player_joined_since_last_check', False) or
        runtime_state.get('player_joined_since_last_backup', False)
    )
