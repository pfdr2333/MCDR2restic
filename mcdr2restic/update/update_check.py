...
class UpdateChecker:
    def __init__(
        ...
        self.thread = threading.Thread(
            target=self._main, name="MCDR2Restic-UpdateCheck", daemon=True
        )
    ...
    def _main(self):
        ...
        while not self.stop_event.is_set():
            try:
                wait_seconds, due_text = compute_update_check_wait_seconds(
                    self.config_provider()
                )
            except Exception as exc:
                self.server.logger.warning(
                    server_tr(self.server, "warn.update.compute_next_failed", error=exc)
                )
                self._wait(60)
                continue
    ...
    def check_now(self, reason: str):
        ...
        update_cfg = (
            cfg.get("update_check", {})
            if isinstance(cfg.get("update_check"), dict)
            else {}
        )
        ...
        try:
            ...
            latest_url = str(
                latest.get("html_url")
                or update_cfg.get("release_page_url")
                or PLUGIN_REPOSITORY_URL
            )
            self._log_check_result(reason, current_version, latest_version, latest_url)
        except Exception as exc:
            self.server.logger.warning(
                server_tr(
                    self.server, "warn.update.check_failed", reason=reason, error=exc
                )
            )

    def _log_check_result(
        self, reason: str, current_version: str, latest_version: str, latest_url: str
    ):
...
def bundled_plugin_metadata_path() -> str:
    ...
    return os.path.abspath(
        os.path.join(
            os.path.dirname(__file__), os.pardir, os.pardir, PLUGIN_METADATA_FILE
        )
    )
...
def fetch_latest_plugin_release(update_cfg: Dict[str, Any]) -> Dict[str, Any]:
    api_url = str(
        update_cfg.get("api_url", DEFAULT_UPDATE_API_URL) or DEFAULT_UPDATE_API_URL
    ).strip()
    timeout = max(3, int(update_cfg.get("timeout_seconds", 10)))
    urls = build_download_urls(
        api_url, update_cfg.get("proxy_prefixes", DEFAULT_PROXY_PREFIXES)
    )
    last_error = ""
    for url in urls:
        try:
    ...
    raise BackupProblem(
        i18n_key="error.update.latest_release_fetch_failed", error=last_error or ""
    )
...
def is_newer_version(latest: str, current: str) -> bool:
    ...
    if latest_tuple or current_tuple:
        ...
        return pad_version_tuple(latest_tuple, width) > pad_version_tuple(
            current_tuple, width
        )
