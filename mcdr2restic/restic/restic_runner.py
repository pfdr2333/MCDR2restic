...
def run_restic_command(
    ...
    progress = build_restic_progress_state(phase, get_mcdr_language(app_runtime.service.server), started)
    ...
    assert_restic_process_completed(app_runtime, restic_cfg, phase, started, stdout, stderr, timed_out)
    maybe_emit_restic_progress(app_runtime.service.server, progress, progress_interval, force=True)
    return build_restic_command_result(phase, args, return_code, stdout, stderr, started, progress)
...
def consume_restic_output_item(
    ...
    wait = compute_restic_queue_wait(progress, progress_interval, get_deadline_remaining(deadline))
    try:
...
def prepare_restic_args_for_phase(args: List[str], phase: str) -> List[str]:
    ...
    if command_name not in args:
        return args
    if any(item == RESTIC_OPTION_JSON or item.startswith(RESTIC_OPTION_JSON + '=') for item in args):
        return args
...
def get_restic_progress_interval(restic_cfg: Dict[str, Any]) -> float:
    try:
        value = float(restic_cfg.get(RESTIC_CFG_PROGRESS_INTERVAL, RESTIC_PROGRESS_INTERVAL_SECONDS))
    except Exception:
