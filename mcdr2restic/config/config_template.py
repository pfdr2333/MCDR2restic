...
def get_schedule_activity_lines(language: str, schedule: Dict[str, Any]) -> List[str]:
    return get_nested_scalar_lines(
        ...
        section_value(language, 'schedule', schedule, 'require_player_activity_in_wait_period')
    )
...
def get_force_schedule_lines(language: str, force_schedule: Dict[str, Any]) -> List[str]:
    ...
    lines = rewrite_nested_scalar(lines, 'interval_seconds', values.get('interval_seconds'))
    return rewrite_nested_scalar(lines, 'cron_expression', values.get('cron_expression'))
...
def get_update_check_lines(language: str, update_check: Dict[str, Any]) -> List[str]:
    ...
    for key in ['enabled', 'check_on_startup', 'daily_time', 'api_url', 'release_page_url', 'timeout_seconds']:
        lines = rewrite_nested_scalar(lines, key, values.get(key))
    return rewrite_nested_sequence(lines, 'proxy_prefixes', values.get('proxy_prefixes'))
...
def get_restic_download_proxy_lines(language: str, restic: Dict[str, Any]) -> List[str]:
    ...
    return rewrite_nested_sequence(lines, 'download_proxy_prefixes', values.get('download_proxy_prefixes'))
...
def get_discord_block_lines(language: str, discord: Dict[str, Any]) -> List[str]:
    ...
    for key in ['enabled', 'webhook_url', 'username', 'avatar_url', 'message_prefix', 'mention_everyone', 'send_timeout_seconds']:
        lines = rewrite_nested_scalar(lines, key, values.get(key))
    lines = rewrite_nested_sequence(lines, 'mention_user_ids', values.get('mention_user_ids'))
    return rewrite_nested_sequence(lines, 'mention_role_ids', values.get('mention_role_ids'))
...
def get_restore_block_lines(language: str, restore_cfg: Dict[str, Any]) -> List[str]:
    ...
    for key in ['pre_restore_backup_tag', 'stop_timeout_seconds', 'start_timeout_seconds']:
        lines = rewrite_nested_scalar(lines, key, values.get(key))
    return lines


def get_restic_scalar_lines(language: str, restic: Dict[str, Any], key: str) -> List[str]:
    return get_nested_scalar_lines(language, 'restic', key, section_value(language, 'restic', restic, key))


def get_nested_scalar_lines(language: str, block_key: str, nested_key: str, value: Any) -> List[str]:
    lines = get_nested_entry_lines(language, block_key, nested_key)
    return rewrite_nested_scalar(lines, nested_key, value)


def section_values(language: str, section_key: str, overrides: Dict[str, Any]) -> Dict[str, Any]:
...
def section_value(language: str, section_key: str, overrides: Dict[str, Any], value_key: str) -> Any:
