...
class CronExpression:
    def __init__(self, expression: str):
        ...
        self.weekdays, self.any_weekday = self._parse_field(
            fields[5], 0, 7, sunday_7=True
        )

    @staticmethod
    def _parse_field(
        text: str, minimum: int, maximum: int, sunday_7: bool = False
    ) -> Tuple[Set[int], bool]:
        values: Set[int] = set()
        wildcard = text == "*"
    ...
    def iter_candidate_times_on_day(
        self, start: datetime, day: date
    ) -> Iterator[datetime]:
    ...
    def iter_candidate_minutes_on_hour(
        self, start: datetime, day: date, hour: int
    ) -> Iterator[datetime]:
    ...
    def iter_candidate_seconds_on_minute(
        self, start: datetime, day: date, hour: int, minute: int
    ) -> Iterator[datetime]:
...
def iter_field_values(
    part: FieldPart, minimum: int, maximum: int, sunday_7: bool
) -> Iterator[int]:
