    ...
    def _send_message(self, text: str):
        ...
        if not webhook_url:
            self.server.logger.warning(
                server_tr(self.server, "warn.discord.webhook_url_empty")
            )
            return
        ...
        try:
            with urllib.request.urlopen(request, timeout=timeout) as response:
                ...
                if status < 200 or status >= 300:
                    self.server.logger.warning(
                        server_tr(
                            self.server, "warn.discord.status_error", status=status
                        )
                    )
        except Exception as exc:
            self.server.logger.warning(
                server_tr(self.server, "warn.discord.send_failed", error=exc)
            )
...
def build_discord_mentions(cfg: Dict[str, Any]) -> List[str]:
    ...
    mentions.extend(
        "<@&{}>".format(value)
        for value in clean_id_list(cfg.get("mention_role_ids", []))
    )
    mentions.extend(
        "<@{}>".format(value)
        for value in clean_id_list(cfg.get("mention_user_ids", []))
    )
    return mentions
