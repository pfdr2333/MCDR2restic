...
class OneBotClient:
    ...
    def start(self):
        ...
        if self.websocket_client is None:
            self.server.logger.warning(
                server_tr(self.server, "warn.onebot.websocket_unavailable")
            )
            return
        self.thread = threading.Thread(
            target=self._thread_main, name="MCDR2Restic-OneBot", daemon=True
        )
        self.thread.start()
    ...
    def _close_websocket(self):
        ...
        try:
            ...
            self.server.logger.debug(
                server_tr(self.server, "debug.onebot.close_error", error=exc)
            )
    ...
    def _connect_once(self):
        ...
        app = None
        try:
            self.websocket_client.setdefaulttimeout(
                float(self.cfg.get("connect_timeout_seconds", 10))
            )
            ...
            if not self.stop_event.is_set():
                self.server.logger.warning(
                    server_tr(self.server, "warn.onebot.connection_error", error=exc)
                )
        finally:
    ...
    def _build_open_callback(self, url: str):
        def on_open(ws):
            ...
            self.server.logger.info(
                server_tr(self.server, "info.onebot.connected", url=self._safe_url(url))
            )
    ...
    def _handle_connection_error(self, _ws, error):
        if not self.stop_event.is_set():
            self.server.logger.warning(
                server_tr(self.server, "warn.onebot.connection_error", error=error)
            )
    ...
    def _send_private_msg(self, user_id: int, text: str):
        ...
        if not self.connected_event.wait(timeout=timeout):
            self.server.logger.warning(
                server_tr(
                    self.server, "warn.onebot.send_not_connected", user_id=user_id
                )
            )
            return
        ws = self.ws
        ...
        try:
            with self.send_lock:
                ws.send(
                    json.dumps(
                        build_private_message_action(user_id, text), ensure_ascii=False
                    )
                )
        except Exception as exc:
            self.server.logger.warning(
                server_tr(
                    self.server, "warn.onebot.send_failed", user_id=user_id, error=exc
                )
            )
